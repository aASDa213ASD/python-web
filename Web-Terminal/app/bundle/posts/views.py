...
from .forms         import (
    CreatePostForm, UpdatePostForm, CreateCategoryForm, 
    UpdateCategoryForm, CreateTagForm, UpdateTagForm, 
    ALLOWED_EXTENSIONS, UPLOAD_FOLDER, MAX_IMAGE_SIZE
)
...
def create_post():
    ...
    if form.validate_on_submit():
        ...
        new_post = Post(title=title, text=text, type=type, image_file=image_path,
...
def view_posts():
    ...
    posts = Post.query.order_by('id', 'created').paginate(page=page, per_page=5)
...
def view_post(id):
    ...
    category = (Category.query.get_or_404(post.category_id)).name
    ...
    if form.validate_on_submit():
        ...
        if new_image_file:
            ...
            if image_path:
                ...
                if full_old_image_path and os.path.isfile(full_old_image_path) and current_post_image != 'default_post.png':
...
def view_tags():
    page = request.args.get('page', 1, type=int)
    tags = Tag.query.paginate(page=page, per_page=5)
...
def view_tag(id):
    ...
    posts = Post.query.join(post_tags).filter(post_tags.c.tag_id == id).all()
